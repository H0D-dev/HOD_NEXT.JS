...
export default function OrderSummary() {
  ...
  return (
    <div className="sticky top-[100px] bg-[var(--surface-primary)] border border-[var(--border-secondary)] p-6 md:p-8 flex flex-col gap-6 max-h-[calc(100vh-120px)] overflow-y-auto hide-scrollbar w-full">
      <h3 className="font-sans text-xl lg:text-2xl font-light text-[var(--text-primary)] m-0 pb-4 border-b border-[var(--border-secondary)]">
        Order Summary
      </h3>
      ...
      <div className="flex flex-col gap-4">
        <div className="flex justify-between items-center font-sans text-sm">
          ...
          <span className="text-[var(--text-primary)] font-sans text-[15px] font-semibold tracking-tight">{formatPrice(subtotal, cartCurrency || "AED")}</span>
        </div>
        <div className="flex justify-between items-center font-sans text-sm">
      ...
      <div className="flex justify-between items-center pt-5 border-t border-[var(--border-secondary)]">
        ...
        <span className="font-sans text-xl md:text-2xl text-[var(--text-primary)] font-medium tracking-tight">{formatPrice(subtotal, cartCurrency || "AED")}</span>
      </div>

      <div className="flex flex-col gap-3 mt-4">
        <Link 
          href="/checkout" 
          className="w-full py-[12px] px-[32px] bg-[var(--accent-primary)] text-[#111] border-[0.5px] border-[var(--border-primary)] font-sans text-[10px] sm:text-[11px] font-semibold uppercase tracking-[0.2em] text-center transition-colors hover:bg-[var(--accent-secondary)]"
        >
          Proceed to Checkout
        </Link>
        <Link 
          href="/products" 
          className="w-full py-[12px] px-[32px] bg-transparent text-[var(--text-primary)] border-[0.5px] border-[var(--border-primary)] font-sans text-[10px] sm:text-[11px] font-semibold uppercase tracking-[0.2em] text-center transition-colors hover:bg-[var(--bg-secondary)]"
        >
          Continue Shopping
        </Link>
      </div>
