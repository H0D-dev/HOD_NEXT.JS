...
export async function GET(request: Request) {
  ...
  try {
    ...
    const data = await analyticsQueryLayer.getVisualizerData(dateRange);
    ...
    console.error("[API /analytics/visualizer] Error:", error);
    return NextResponse.json(
      { success: false, error: "Failed to generate visualizer analytics" },
      { status: 500 }
    );
  }
}
