...
export async function GET(request: Request) {
  ...
  try {
    ...
    const data = await analyticsQueryLayer.getBehaviorData(dateRange);
    ...
    console.error("[API /analytics/behavior] Error:", error);
    return NextResponse.json(
      { success: false, error: "Failed to generate behavior analytics" },
      { status: 500 }
    );
  }
}
