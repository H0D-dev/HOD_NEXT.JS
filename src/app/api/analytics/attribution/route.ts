...
export async function GET(request: Request) {
  ...
  try {
    ...
    const data = await analyticsQueryLayer.getAttributionData(dateRange);
    ...
    console.error("[API /analytics/attribution] Error:", error);
    return NextResponse.json(
      { success: false, error: "Failed to generate attribution analytics" },
      { status: 500 }
    );
  }
}
